...
### M.Tech Exam-Ready Notes (Units I–V)

> **How to use this file:** Every topic opens with a one-line definition (write this first in the exam), followed by an explanation in plain English (what to *write* in your answer), then diagrams/examples/tables as supporting evidence. Always structure long answers as: Definition → Explanation → Diagram/Example → Advantages/Comparison (if applicable).
...
├── Entity-Relationship Model → data described via entities, attributes, and relationships (used for design, Section 1.7)
...
**Explain why we need this:** Finding the irreducible set (also used to compute **candidate keys** and to test **dependency preservation** during normalization, Section 3.3) removes redundant dependencies so that database design decisions are based only on the essential, non-redundant relationships between attributes.
...
**Definition:** A join dependency `*(R1, R2, ..., Rn)` on relation R holds if R can be losslessly reconstructed by joining R1, R2, ..., Rn back together — this is a generalization of the lossless decomposition concept (Section 3.2) to **more than two** relations.
...
| **Graph Database** | Nodes and edges (as covered in Section 5.1) | Neo4j, ArangoDB | Highly interconnected data — social networks, recommendations |
...
- [ ] Advantages of DBMS over file-processing (all 7 points)
- [ ] Three-schema architecture diagram + physical vs logical data independence
- [ ] Types of keys: super, candidate, primary, alternate, foreign (with definitions)
- [ ] Weak vs Strong entity sets + example
- [ ] Specialization vs Generalization vs Aggregation vs Inheritance
- [ ] Rules for reducing ER schema to tables (all 6 cases: strong entity, weak entity, 1:1, 1:M, M:N, specialization)
- [ ] Relational Algebra operators (σ, π, ∪, −, ×, ⋈) with symbols and examples
- [ ] Relational Algebra vs Relational Calculus comparison table
- [ ] WHERE vs HAVING; correlated vs non-correlated subquery
- [ ] Types of SQL JOINs with Venn diagrams
- [ ] DELETE vs TRUNCATE vs DROP
- [ ] Armstrong's Axioms (Reflexivity, Augmentation, Transitivity)
- [ ] Insertion/Deletion/Update anomalies with example
- [ ] 1NF → 2NF → 3NF → BCNF → 4NF → 5NF (condition for each, in order)
- [ ] 3NF vs BCNF comparison
- [ ] Entity Integrity Rule vs Referential Integrity Rule
- [ ] Triggers: event-condition-action, BEFORE/AFTER, ROW/STATEMENT
- [ ] ACID properties (all four, with one example each)
- [ ] Transaction state diagram (Active → Partially Committed → Committed/Failed → Aborted)
- [ ] Lost Update, Dirty Read, Incorrect Summary, Unrepeatable Read problems
- [ ] Conflict serializability via precedence graph (practice a numerical example)
- [ ] Deadlock: prevention vs detection-and-recovery
- [ ] Deferred vs Immediate database modification (UNDO/REDO requirement)
- [ ] Checkpoints — why they're needed
- [ ] Steps in query processing: Parsing → Optimization → Evaluation
- [ ] CAP theorem (Consistency, Availability, Partition tolerance) + why P is non-negotiable
- [ ] 4 types of NoSQL databases with one example DB each
- [ ] SQL vs NoSQL comparison table
- [ ] ACID vs BASE
- [ ] Valid time vs Transaction time (temporal databases)
- [ ] Graph database: nodes, edges, index-free adjacency
- [ ] Spatial database: R-tree/Quad-tree indexing, spatial query types

---

*CS-6418: Advanced Database Management System | M.Tech Exam-Ready Notes | Units I–V*
