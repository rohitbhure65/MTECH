...
1. [Unit I — Introduction to Information System Design](#unit-i)
2. [Unit II — Project Management and Planning](#unit-ii)
3. [Unit III — Requirements Engineering & Software Design](#unit-iii)
4. [Unit IV — Object-Oriented Analysis, Design & UML](#unit-iv)
5. [Unit V — Implementation and Testing](#unit-v)
6. [Quick Revision Sheet](#quick-revision)
...
5. **Cost & time overruns** — most famous cause of project failure (see Unit II).
...
- [IBM RUP Overview](https://www.ibm.com/docs/en/rational-soft-arch/9.6.1?topic=rup-rational-unified-process)
- GeeksforGeeks: search "Software Process Models" and "Component Based Software Engineering" for quick diagram-based revision.
...
- [COCOMO Model explained — GeeksforGeeks](https://www.geeksforgeeks.org/software-engineering-cocomo-model/)
- Standish Group **CHAOS Report** (search "CHAOS Report software project success statistics").
- [PERT vs Gantt chart — comparison articles on ProjectManager.com](https://www.projectmanager.com/)
...
- [GeeksforGeeks: Coupling and Cohesion in Software Engineering](https://www.geeksforgeeks.org/coupling-and-cohesion-software-engineering/)
- [GeeksforGeeks: Requirements Engineering Process](https://www.geeksforgeeks.org/software-engineering-requirements-engineering-process/)
...
- [UML Diagrams — Official OMG UML Specification](https://www.omg.org/spec/UML/)
- [Visual Paradigm — UML Diagram Tutorials with examples](https://www.visual-paradigm.com/guide/uml-unified-modeling-language/)
- [Lucidchart — UML Diagram types explained visually](https://www.lucidchart.com/pages/uml-diagram)
...
- [GeeksforGeeks: Levels of Software Testing](https://www.geeksforgeeks.org/levels-of-software-testing/)
- [Selenium official documentation](https://www.selenium.dev/documentation/)
- [Martin Fowler on TDD — martinfowler.com](https://martinfowler.com/bliki/TestDrivenDevelopment.html)
...
**Unit I:** IS = People + Hardware + Software + Data + Procedures. Models: Waterfall (sequential), CBD (reuse components), Agile (iterative sprints), RUP (4 phases: Inception-Elaboration-Construction-Transition).

**Unit II:** Iron Triangle = Scope-Cost-Time-Quality. Brooks's Law = adding people to a late project makes it later. COCOMO = Effort = a×(KLOC)^b, types: Organic/Semi-detached/Embedded. Gantt = timeline; PERT/CPM = dependencies + critical path. SCM = version control + change control. Risk cycle = Identify→Analyze→Prioritize→Mitigate→Monitor.

**Unit III:** RE cycle = Elicitation→Analysis→Specification→Validation (+Management). Structured (DFD, process-first) vs OO (UML, object-first). SRS must be: Correct, Unambiguous, Complete, Consistent, Verifiable, Traceable. Design goals: LOW coupling, HIGH cohesion. Refactoring = change structure, NOT behavior.

**Unit IV:** UML = Structural (Class, Object, Component, Deployment, Package) + Behavioral (Use Case, Sequence, Collaboration, State-Chart, Activity). Use Case: include=mandatory, extend=optional. Class relations: Association < Aggregation (weak has-a) < Composition (strong has-a); Generalization = is-a. Sequence = time-order; Collaboration = structure/links. Activity = workflow; State-Chart = object lifecycle. Component = logical software view; Deployment = physical hardware view.

**Unit V:** TDD cycle = Red→Green→Refactor. Black-box = external/functional; White-box = internal/code-path. Testing pyramid = Unit→Integration→System→Acceptance. Integration approaches = Top-down/Bottom-up/Big-Bang. Debugging ≠ Testing (testing finds bugs, debugging finds root cause + fixes).
...
- For "case study/design a system" questions: follow the structured approach given at the end of Unit I (§1.5) and Unit IV (§4.11).
- Practice DRAWING every diagram in Unit IV by hand at least 2–3 times before the exam — diagram-based questions carry heavy marks and hand-drawing builds muscle memory.
